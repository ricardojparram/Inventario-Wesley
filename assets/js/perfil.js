$(document).ready(function () {
  ...
  function mostrarUsuarios() {
    $.post("", { lista: "mostrar", usuarios: "usuarios" }, function (response) {
  ...
  $("#nameEdit").keyup(() => {
    validarNombre($("#nameEdit"), $("#errorNom"), "Error de nombre,");
  });
  $("#apeEdit").keyup(() => {
    validarNombre($("#apeEdit"), $("#errorApe"), "Error de apellido,");
  });
  $("#cedulaEdit").keyup(() => {
    ...
    timeout = setTimeout(function () {
      if (valid) {
        validarCedulaBD($("#cedulaEdit"), $("#errorCedu"), $("#preDocument"));
      }
    }, 700);
  });
  $("#emailEdit").keyup(() => {
    let valid = validarCorreo(
      $("#emailEdit"),
      $("#errorEmail"),
      "Error de email,"
    );
    ...
    validarCedulaBD($("#cedulaEdit"), $("#errorCedu"), $("#preDocument")).then(
      () => {
        validarCorreoBD($("#emailEdit"), $("#errorEmail")).then(() => {
          ...
          form.append(
            "cedula",
            $("#preDocument").val() + "-" + $("#cedulaEdit").val()
          );
          let borrar = $("#imgEditar").is(
  ...
  $("#editContra").click(function (e) {
    e.preventDefault();
    reContra = validarRepContraseña(
      $("#rePassword"),
      $("#rePasswordError"),
      $("#newPassword")
    );
    contra = validarContraseña(
      $("#newPassword"),
      $("#newPasswordError"),
      "Error de Contraseña Nueva,"
    );
    actContra = validarContraseña(
      $("#password"),
      $("#passwordError"),
      "Error de Contraseña Actual,"
    );
  ...
  async function validarCorreoBD(input, div) {
    ...
    await $.getJSON(
      "",
      { correo: input.val(), validarCorreo: "" },
      function (res) {
        div.text("");
        input.removeClass("input-error");
        resultado = true;

$(document).ready(function () {
      ...
      success(data) {
        ...
        data.forEach((row) => {
          tabla += `
              ...
              <button type="button" id="${row.cod_pres}" class="btn btn-registrar editar" data-bs-toggle="modal" data-bs-target="#editModal"><i class="bi bi-pencil"></i></button>
  ...
  $("#medida").change(function () {
    validarSelect($("#medida"), $("#error"), "Error, escoge una medida");
  });
  $("#cantidad").keyup(() => {
    validarNumero($("#cantidad"), $("#error"), "Error en Cantidad,");
  });
  $("#peso").keyup(() => {
    validarNumero($("#peso"), $("#error"), "Error en Peso,");
  });

  $("#registrar").click((e) => {
    let vmedida = validarSelect(
      $("#medida"),
      $("#error"),
      "Error tipo producto",
    );
    let vcantidad = validarNumero(
      $("#cantidad"),
      $("#error"),
      "Error en Cantidad,",
    );

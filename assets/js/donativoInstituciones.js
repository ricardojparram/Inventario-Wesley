       ...
       $.post('',{detalleD : 'detalle Donacion' , id}, function(data){
   		...
   		function selectInstituciones() {
   			$.ajax({
   				...
   				url: '',
   				dataType: 'json',
   				data: { selectInstituciones: 'instituciones' },
   				success(data) {
   					$('#rif').select2({
   					...
   					$('#rif').on('change', function() {
   						let selectedRif = $(this).val();
   						let institucion = data.find(item => item.rif_int === selectedRif)
   						if (institucion){
   							let RazonSocial = institucion.razon_social;
   							$('#beneficiario').val(RazonSocial);
   						}
   					});
   				}
   			});

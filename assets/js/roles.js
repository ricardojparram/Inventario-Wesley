$(document).ready(function () {
  ...
  const rellenar = (bitacora = false) => {
    $.post(
      ...
      function (data) {
        let tabla;
        const permisoAcciones = !permisos["Modificar acciones"]
          ? "disabled"
          : "";
      ...
      function (data) {
        ...
        Object.entries(data).forEach(([modulo_nombre, row]) => {
          ...
          Object.entries(row).forEach(([nombre_permiso, permiso]) => {
            ...
            let title =
              nombre_permiso == "Consultar" ? "Acceso" : nombre_permiso;
            permisos += `
  ...
  $("#enviarPermisos").click(() => {
    ...
    $("#tabla_permisos td input").each(function (i) {
      ...
      datos_permisos[i] = {
        id_permiso: input_permiso.id,
        status: input_permiso.checked,
      };
    });
    $(this).prop("disabled", true);
    $.post(

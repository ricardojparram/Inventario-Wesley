<?php
  ...
  public function getEditarProd($cod_productoEd, $tipoprodEd, $fechaEd ,$composicionEd,$posologiaEd,$ubicacionEd,$laboratorioEd,$presentacionEd,$tipoEd,$claseEd,$contraInEd,$id){

    if(preg_match_all("/^[a-zA-ZÀ-ÿ]+([a-zA-ZÀ-ÿ0-9\/\s()#,.-]){3,200}$/", $tipoprodEd) !== 1){
      return ['resultado' => 'error', 'error' => 'Descripcion inválida'];
    }
    ...
    $this->id = $id;
    $this->cod_producto = $cod_productoEd;
    $this->tipoprodEd = $tipoprodEd;
  ...
  private function editarProd(){
    try{
      parent::conectarDB();
      $new = $this->con->prepare("UPDATE producto p SET `cod_producto`= ? ,`id_tipoprod`= ? , ,`composicion`= ? ,`contraindicaciones`= ? ,`posologia`= ?  ,`rif_laboratorio`= ? ,`id_tipo`= ? ,`id_clase`= ? ,`cod_pres`= ? WHERE p.status = 1 AND p.cod_producto = ?");

      $new->bindValue(1, $this->cod_producto);
      $new->bindValue(2, $this->tipoprod);
      $new->bindValue(3, $this->presentación);
      $new->bindValue(4, $this->laboratorio);
      $new->bindValue(5, $this->tipoP);
      $new->bindValue(6, $this->clase);
      $new->bindValue(7, $this->composicionP); 
      $new->bindValue(8, $this->posologia);
      $new->bindValue(9, $this->contraIn);
